...
for x=matfilelist
    ...
    for indx=1:size(K.SPK,1)
        ...
        if length(x)>0
            ...
            slice1=slice1+1;
            if strcmp(type,'WT')
                WT_auROC_FR(end+1,:)={K.date,K.ID{1,1},slice1,(length(x)/(K.Event{1,4}(2)-K.Event{1,4}(1)))*10^6,C',S'};
            
                WTnum=num;
            else
                KO_auROC_FR(end+1,:)={K.date,K.ID{1,1},slice1,(length(x)/(K.Event{1,4}(2)-K.Event{1,4}(1)))*10^6,C',S'};
                KOnum=num;
            end            
        end
    ...
    for indx=1:size(K.SPK,1)
        ...
        if length(x)>0
            x=x-K.Event{1,4}(1)/1000000;
            num=num+1;
            sbehavior=[K.SocialBehavior{1,2}{1,1};K.SocialBehavior{1,2}{1,2};K.SocialBehavior{1,2}{1,3};K.SocialBehavior{1,2}{1,4}];
            ...
            slice2=slice2+1;
            if strcmp(type,'WT')
                WT_auROC_FR(end+1,:)={K.date,K.ID{1,2},slice2,(length(x)/(K.Event{1,4}(2)-K.Event{1,4}(1)))*10^6,C',S'};
            
                WTnum=num;
            else
                KO_auROC_FR(end+1,:)={K.date,K.ID{1,2},slice2,(length(x)/(K.Event{1,4}(2)-K.Event{1,4}(1)))*10^6,C',S'};
                KOnum=num;
            end
        end
...
save('auROC__social_neuron_other_auROC','WT_auROC_FR','KO_auROC_FR','WT_AUC', 'WT_rand_AUC_temp', 'WT_AUC_temp', 'KO_AUC', 'KO_rand_AUC_temp', 'KO_AUC_temp','WT_social_neuron','WT_pos_social_neuron','WT_neg_social_neuron','KO_social_neuron','KO_pos_social_neuron','KO_neg_social_neuron','WT_S_larger_than_criteria', 'WT_C_larger_than_criteria', 'WT_rand_S_larger_than_criteria', 'WT_rand_C_larger_than_criteria', 'KO_S_larger_than_criteria', 'KO_C_larger_than_criteria', 'KO_rand_S_larger_than_criteria', 'KO_rand_C_larger_than_criteria')
