...
for ii = 10%:length(foldernames)
    ...
    for i=1:length(A)
        ...
        for k=1:size(LFP_Samples,2)
            LFP((512*(k-1)+1):(512*(k-1)+512),17)=LFP_Timestamps(k):533:(LFP_Timestamps(k)+533*511); %1/1875*10^6 (1/Fs*10^6)
        end
    end
    ...
    LFP_directinteraction_mouse2 = LFP((directinteraction_ts_indices(1):directinteraction_ts_indices(end)),[1:1:8]);
    LFP_directinteraction_mouse1 = LFP((directinteraction_ts_indices(1):directinteraction_ts_indices(end)),[9:1:16]);
    ...
    K.LFP {2,2} = LFP_directinteraction_mouse1;
    K.LFP {2,1} = LFP_directinteraction_mouse2;
    ...
    K.NonSocialBehavior{:,1} = {'grooming','rearing','jumping','exploring','moving'};
    K.NonSocialBehavior{:,2} = {grooming_data,rearing_data,jumping_data,circling_data,exploring_data};
    ...
    K.NonSocialBehavior{:,3} = {grooming_data,rearing_data,jumping_data,circling_data,exploring_data};
