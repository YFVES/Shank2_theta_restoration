...
%m1
%exploring
Total{l,2}=exploring_dat(l,1);
Total{l,3}=exploring_dat(l,2);
Total{l,4}=mean(exploringraw{1,l}(:,3));

%jumping
Total{l,5}=jumping_dat(l,1);
Total{l,6}=jumping_dat(l,2);
Total{l,7}=mean(jumpingraw{1,l}(:,3));

%rearing
Total{l,8}=rearing_dat(l,1);
Total{l,9}=rearing_dat(l,2);
Total{l,10}=mean(rearingraw{1,l}(:,3));

%circling
Total{l,11}=circling_dat(l,1);
Total{l,12}=circling_dat(l,2);
Total{l,13}=mean(circlingraw{1,l}(:,3));

%grooming
Total{l,14}=grooming_dat(l,1);
Total{l,15}=grooming_dat(l,2);
Total{l,16}=mean(groomingraw{1,l}(:,3));

%rest
Total{l,17}=rest_dat(l,1);
Total{l,18}=rest_dat(l,2);
Total{l,19}=mean(restraw{1,l}(:,3));



grooming=[]; rearing=[]; jumping=[]; circling = []; exploring = []; rest = []; 
