function plotLfpCorrelationWithBehavior(lfp_mouse1, lfp_mouse2, Fs, freqBands, smoothingWindow,S_NS_Vector, ii)
...
for band = 1:length(freqBands)
    ...
    % Calculate z-scores for the plot data
    zScores = (plotData - mean(plotData)) / std(plotData);
    ...
    % Mark values where z-score is above 2
    hold on;
    highZScoreIndices = find(zScores > 2);
    plot(highZScoreIndices, plotData(highZScoreIndices), 'r.', 'MarkerSize', 10);
